...
public class SpringIndicator extends FrameLayout
{
	...
	private void setUpListener()
	{
		viewPager.setOnPageChangeListener(new ViewPager.SimpleOnPageChangeListener()
		{
					...
					int positionOffsetPixels)
			{
				if (position < tabs.size() - 1)
				{
					...
					if (positionOffset < headMoveOffset)
					{
						...
						headX = (float) ((Math.atan(
								positionOffsetTemp * acceleration * 2 - acceleration) +
								(Math.atan(acceleration))) / (2 * (Math.atan(acceleration))));
					}
					springView.getHeadPoint()
							.setX(getTabX(position) - headX * getPositionDistance(position));
					float footX = 0f;
					...
					springView.getFootPoint()
							.setX(getTabX(position) - footX * getPositionDistance(position));
				...
				{
					float length =
							(position + positionOffset) / viewPager.getAdapter().getCount();
	...
	public void updatePointerUI()
	{
		...
		if (springView != null)
		{
			this.removeView(springView);
		}
		if (tabContainer != null)
		{
			this.removeView(tabContainer);
		}
		initSpringView();
		setUpListener();
	}
	...
	{
		indicatorColorAnim =
				ObjectAnimator.ofInt(springView, "indicatorColor", indicatorColorArray);
